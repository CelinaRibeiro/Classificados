...
class Anuncios{
    ...
    public function getUltimosAnuncios($page, $perPage, $filtros){
        ...
        //iniciar a paginação da pg 1, multiplica pela qtdade que ficará na pg
        ...
        $sql = $pdo->prepare("SELECT *,
                            ...
                            (select categorias.nome from categorias where categorias.id = 
                                anuncios.id_categoria) as categoria FROM anuncios
                                WHERE ".implode(' AND ', $filtrostring)."
        ...
        if(count($fotos) > 0) {
            for($q=0;$q<count($fotos['tmp_name']);$q++){
                ...
                if (in_array($tipo, array('image/jpeg', 'image/png'))) {
                    //gera um nome aleatório com extensão jpg e salva os arquivos
                    $tmpname = md5(time().rand(0, 9999)).'.jpg';
                    move_uploaded_file($fotos['tmp_name'][$q], 'assets/images/anuncios/'.$tmpname);
                    
                    //redimensionamento da img, salvando nas variaveis a altura e o tam original
                    ...
                    //insere a img original dentro da nova imagem
                    imagecopyresampled($img, $origi, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
                    
                    //salva a imagem
                    imagejpeg($img, 'assets/images/anuncios/'.$tmpname, 80);
                    
                    //depois de salvo a img no servidor salva a img no bd
